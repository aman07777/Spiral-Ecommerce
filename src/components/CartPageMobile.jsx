...
import { Flex, Box, Heading, Button, Checkbox, Input, Image, Text } from '@chakra-ui/react';
...
function CartPageMobile() {
  ...
  const handleSelectItem = (event, index) => {
    ...
    setIsDeleteAllVisible(newCartItems.filter((item) => item.isChecked).length === newCartItems.length);
  };
  ...
  return (
    <>
      ...
      <Flex direction={{ base: 'column', md: 'row' }} p={4}>
        <Flex direction="column" mb={4} mr={{ md: 4 }} flex={{ base: 'none', md: 1 }}>
          <Flex direction="column">
            <Flex direction="row" justifyContent="space-between" alignItems="center" mb={2}>
              <Checkbox isChecked={isSelectAllChecked} onChange={handleSelectAll}>
                Select All
              </Checkbox>
              {isDeleteAllVisible && (
            ...
            {cartItems.map((item, index) => (
              <Box key={item.id} borderWidth="1px" borderRadius="lg" overflow="hidden" boxShadow="md" mb={4}>
                <Flex direction="column">
                  <Flex direction="row" justifyContent="space-between" alignItems="center" p={2}>
                    <Box pb={20}>
                    <Checkbox isChecked={item.isChecked} onChange={(event) => handleSelectItem(event, index)} />
                    </Box>
                    <Image src={item.image} alt="Product Image" boxSize="50px" mr={4} borderRadius="0.5rem" />
                    <Box>
            ...
            {cartItems.length === 0 && (
              <Box borderWidth="1px" borderRadius="lg" overflow="hidden" boxShadow="md" p={2} textAlign="center">
                Your cart is empty
              </Box>
            )}
          </Flex>
        </Flex>
        <Box borderWidth="1px" borderRadius="lg" overflow="hidden" p={4} boxShadow="md"  width={{base:'none',md:"27vw"}}>
          <Flex direction="column">
            <Box mb={2}>
            ...
            <Box mb={2}>
              <Flex direction="row" justifyContent="space-between">
                ...
                <Text fontSize="md" fontWeight="bold">${grandTotal.toFixed(2)}</Text>
              </Flex>
            </Box>

            <Flex direction="column">
              <Input type="text" placeholder="Enter voucher code" size="sm" borderRadius="0.5rem" border="1px solid black" color="black" bg="white" mb={2} />
              <Button size="sm" borderRadius="0.5rem" bg="white" color="black" _hover={{ bg: 'black', color: 'white' }} mb={2}>
                Apply Voucher
              </Button>
              <Button colorScheme="blue" size="lg" onClick={handleCheckout} isFullWidth borderRadius="0.5rem">
                Checkout now
              </Button> 
            </Flex>
          </Flex>
