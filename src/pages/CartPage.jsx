...
import { Flex, Box, Button, Table, Thead, Tbody, Tr, Th, Td, Checkbox, Input, Image, Text, useToast } from '@chakra-ui/react';
...
function CartPage() {
  ...
  useEffect(() => {
    getCart(currentUser).then((result) => {
  ...
  const handleRemoveCartItem = (productId) => {
    removeCartItem(currentUser, productId).then((result) => {
      toast({
  ...
  return (
    <>
      <Navbar />
      {!isLoading ? <Flex direction="column" p={4}>
              ...
              <Table variant="simple">
                ...
                <Tbody>
                  {cartItems.map((product, index) => (
                    <Tr key={product.id}>
                      <Td>
                        <Checkbox isChecked={product.isChecked} onChange={(event) => handleSelectItem(event, index)} />
                      </Td>
                      <Td>
                        <Flex align="center">
                          <Image src={`http://localhost:8080/${product.image}`} alt="Product Image" boxSize={["30px", "30px", "50px"]} mr={4} borderRadius="0.5rem" />
                          <Box>
                      ...
                      <Td>
                        <Flex direction={["column", "column", "row"]} align={["flex-start", "flex-start", "center"]}>
                          <Button size="sm" onClick={() => handleQuantityChange({ target: { value: product.selectedQuantity - 1 } }, index)}>-</Button>
                          <Input
                            type="number"
                      ...
                      <Td>${product.price}</Td>
                      <Td>
                        <Button colorScheme="red" size="sm" onClick={() => handleRemoveCartItem(product.id)} borderRadius="2rem">
                          <DeleteIcon />
                        </Button>
                      </Td>
                    </Tr>
