...
import { Box, Text, Input, Button, FormLabel, useToast } from '@chakra-ui/react'
  ...
  return (
    ...
    >  {!showResetPassword ? (
      <>
        ...
        <Input name="email" pr="4.5rem" placeholder="Email" onChange={(e) => { setEmail(e.target.value) }} />
        <Button mt={5} width="100%" colorScheme='linkedin' onClick={handleResetPassword}>Continue</Button>
      ...
      <>
        ...
        <Input name="password" pr="4.5rem" placeholder="New Password" onChange={(e) => { setPassword(e.target.value) }} />

        <FormLabel fontSize={20} margin="1rem 20rem 1rem 0rem">Confirm Password</FormLabel>
        <Input name="password" pr="4.5rem" placeholder="Confirm Password" onChange={(e) => { setConfirmPassword(e.target.value) }} />
        <Button mt={5} width="100%" colorScheme='linkedin' onClick={handleNewPassoword}>Reset Password</Button>
      </>
    )}
    </Box>
    // 
