...
const MyOrder = () => {
    ...
    getMyOrders()
      .then((data) => {
        const deliveredOrders = data.filter(
          (order) => order?.orderItems[0].status === "Delivered"
        );
        setMyOrders(data);
  ...
  const filterOrdersData = useCallback(
    (orders) => {
      let filteredOrder;
      const now = new Date();
        ...
        <div className="w-[100%] md:w-[80%]">
          ...
          >
            <Flex alignItems="center" bg="gray.50" p={4}>
              <TopSide />
              <div className="items-center justify-center hidden w-full gap-x-2 md:flex">
                <span className="text-lg font-bold">My Orders</span>
                <i>
                  <BiSolidCart className="text-[#008080]" size={20} />
                </i>
              </div>
              <Box
            ...
            <div className="flex items-center justify-center w-full gap-x-2 mb-7 md:hidden">
              <span className="text-lg font-bold">My Orders</span>
              <i>
                <BiSolidCart className="text-[#008080]" size={20} />
              </i>
                    ...
                    <div className="flex gap-x-10">
                      <span
                        className={`text-sm cursor-pointer  ${
                          what === "orders" && "border-t-2 border-[#008080]"
                        }`}
                        onClick={(e) => {
                          setWhat("orders");
                        }}
                      ...
                      <span
                        className={`text-sm cursor-pointer ${
                          what === "history" && "border-t-2 border-[#008080]"
                        }`}
                        onClick={() => {
                          setWhat("history");
                        }}
